...
setwd("C:/Users/mlfearon/OneDrive - Umich/PROJECTS/Manuscripts/Nosema and visitation patterns/Data and Code")
...
data <- read.csv("NosemaAnalysis_17Oct2022_18Sremoved.csv", stringsAsFactors = F)
...
a <- emmeans(fit_cat, specs = pairwise ~ Genus|Site, type = "response")
a
...
b <- emmeans(fit_cat, specs = pairwise ~ Site|Genus, type = "response")
b


####CHECK THIS 10.17.2022
...
# fit the model including spatial random effects 
...
# Appendix Table SX
write.csv(pairwise_hosts, file = "UPDATED_10.17.22_Nosema_Pairwise_Tests__ByHostWithinEachSite.csv", quote = F)
...
# Appendix Table SX
write.csv(pairwise_sites, file = "UPDATED_10.17.22_Nosema_Pairwise_Tests__BySiteForEachHost.csv", quote = F)
...
# Appendix Table SX (possibly to go in the main text)
write.csv(PrevByHost, file = "UPDATED_10.17.22_Nosema_Predicted_Prevalence_By_Host_and_Site.csv", quote = F)
...
print(prev_site)
ggsave("Fig1_NosemaPrev_BY_Site_BOTH_Hosts_predict.tiff", plot = prev_site, dpi = 300, width = 12, height = 8, units = "cm", compression="lzw")
...
# Model of visit frequency for Nosema in Apis
...
# Model of visit frequency for Nosema in Bombus
...
prev_ApisNum <- ggplot(me_ApisNum, aes(x = ApisVisits, y = predicted)) +
  ...
  labs(color = "Species", tag = "a", x = bquote(atop("Avg. Number of" ~ italic(Apis),"visits (per 30 min)")), y = bquote(italic(V.) ~ italic(ceranae) ~ " Prevalence")) +
  ylim(0,1) +
  theme_classic() +
...
prev_ApisDur <- ggplot(me_ApisDur, aes(x = ApisDur, y = predicted)) +
  ...
  labs(color = "Species", tag = "b", x = bquote(atop("Avg. Duration of" ~ italic(Apis), "visits (seconds)")), y = bquote(italic(V.) ~ italic(ceranae) ~ " Prevalence")) +
  ylim(0,1) +
  theme_classic() +
  theme(axis.text = element_text(size=8, color = "black"), axis.title = element_text(size=8, color="black"),
                                 legend.text = element_text(size = 8), legend.title = element_text(size = 8))
...
prev_ApisDur_PN <- ggplot(me_ApisDur_PN, aes(x = ApisDurPN, y = predicted)) +
  ...
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = Host_Species), alpha = 0.2, outline.type = NULL) +
  scale_linetype_manual(values=c("dashed", "solid"), guide = "none") +
  labs(color = "Species", tag = "c", x = bquote(atop("Avg. Duration of" ~ italic(Apis), "Pollen + Nectar visits (seconds)")), y = bquote(italic(V.) ~ italic(ceranae) ~ " Prevalence")) +
  ylim(0,1) +
  theme_classic() +
  theme(axis.text = element_text(size=8, color = "black"), axis.title = element_text(size=8, color="black"),
        legend.text = element_text(size = 8), legend.title = element_text(size = 8))
...
ggsave("Fig2.tiff", plot = prev_fig2, dpi = 600, width = 8, height = 3.5, units = "in", compression="lzw")
...
ggsave("Fig4.tiff", plot = prev_fig3, dpi = 600, width = 6.5, height = 3, units = "in", compression="lzw")

# color blind check
cvdPlot(prev_fig3)


check <- glm(VisitDur ~ VisitNum, data = data)
summary(check)
